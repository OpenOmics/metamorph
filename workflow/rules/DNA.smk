...
rule metawrap_read_qc:
    ...
    params:
        ...
        tmpr1               = lambda _, output, input: str(input.R1).replace('_R1.', '_1.'),
        tmpr2               = lambda _, output, input: str(input.R2).replace('_R2.', '_2.')
    containerized: metawrap_container,
    threads: int(cluster["metawrap_genome_assembly"].get('threads', default_threads)),
    shell: 
        """
            # metawrap input needs to be renamed [stem]_1.fastq and [stem]_2.fastq
...
rule metawrap_tax_classification:
    """
        The metaWRAP::Kraken module takes in any number of fastq or fasta files, classifies the contained sequences with KRAKEN, 
        and reports the taxonomy distribution in a kronagram using KronaTools. If the sequences passed to the module belong to an 
        assembly and follow the contig naming convention of the Assembly module, the taxonomy of each contig is weighted based on 
        its length and coverage [weight=coverage*length]. The classifications of the sequences are then summarized in a format 
        that KronaTools' ktImportText function recognizes, and a final kronagram in html format is made with all the samples.
    """
    input:
...
rule metawrap_binning:
    """
        Metawrap wrapper for binning of reads/contigs to assemblies.
        ...
        Stepwise algorithm for metawrap genome binning:
            ...
            2. Collate, sort, stage alignment outputs 
                a. Alignments are sorted and compressed with samtools, and library insert size statistics are also gathered at the same time (insert size average and standard deviation). 
