# <code>metamorph <b>unlock</b></code>
...
This part of the documentation describes options and concepts for <code>metamorph <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking metamorph pipeline output directory is fast and easy! In its most basic form, <code>metamorph <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./metamorph unlock [-h] --output OUTPUT
```
...
metamorph unlock --output /data/$USER/output
```